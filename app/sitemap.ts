...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const sashualoDonisHtmlCssLinks: MetadataRoute.Sitemap = sashualoDonisHtmlCss.map((link) => (
    {
      url: `${process.env.NEXT_PUBLIC_BASE_URL}/courses${link}`,
  ...
  const fundamentalebiLinks: MetadataRoute.Sitemap = fundamentalebi.map((link) => (
    {
      url: `${process.env.NEXT_PUBLIC_BASE_URL}/courses${link}`,
