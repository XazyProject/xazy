...
const LessonLayout: React.FC<any> = ({ urlData }) => {
  // სთეით ცვლადები
    ...
    const fetchMarkdownContent = async () => {
      try {
        const response = await fetch(
          `/masala/${urlData.courseName}/${urlData.chapter}/${urlData.lesson}.md`
        );
  ...
  useEffect(() => {
    ...
    if (h1Element) {
      const pageTitle = `${
        h1Element.textContent || h1Element.innerText
      } | პროგრამირების უფასო კურსი | XAZY`;
